...
public class GLESVertexArrayManager extends VertexArrayManager {
	...
	final public void init(int numFaces) {
		...
		// 6 verteces (2 triangles) with 3 floats for coordinates and 4 color
		// channels. each of those take 4 bytes.
		capacity = numFaces * ((6 * (3 + 4)) * 4);

		vertexByteBuffer = ByteBuffer.allocateDirect(capacity).order(
				ByteOrder.nativeOrder());
		vertexIntBuffer = vertexByteBuffer.asIntBuffer();
		vertexBuffer = vertexByteBuffer.asFloatBuffer();

		colorByteBuffer = ByteBuffer.allocateDirect(capacity).order(
				ByteOrder.nativeOrder());
	...
	final public void pushIntoFrameAsStatic(float[] vertexData,
			float[] colorData) {
		try {
