...
public class TotautisSpaceStationTest {
	...
	public final void testBlowtorch() {
		...
		bw.update( 1 );
		float diff = Math.abs( bw.getFuel() - bw2.getFuel() );
		Assert.assertTrue( diff > BlowTorch.DEFAULT_IDLE_FUEL_USAGE );
		...
		bw.toggle();
		Assert.assertFalse(bw.isActive());
		try {
			plate.useWith(bw);
			Assert.fail();
		} catch (ItemActionNotSupportedException ignored) {
		...
		bw = new BlowTorch(100);
		Assert.assertEquals(100.0f, bw.getFuel(), 0.1f);
		Assert.assertFalse(bw.isDepleted());
		bw.toggle();
		bw.update( 1 );
		Assert.assertTrue(bw.isActive());
		Assert.assertEquals(100.0f - BlowTorch.DEFAULT_IDLE_FUEL_USAGE, bw.getFuel(), 0.1f);
		Assert.assertFalse(bw.isDepleted());
