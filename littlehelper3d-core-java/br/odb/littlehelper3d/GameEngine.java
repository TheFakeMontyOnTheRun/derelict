...
public class GameEngine implements Runnable {
	 ...
	 * @param sectorId
	 */
	public void placeActor(AndroidGameActor actor, int sectorId) {
	...
	// ------------------------------------------------------------------------------------------------------------
	private void doGravity(AndroidGameActor actor) {
	...
	public void run() {
		...
		ArrayList<AndroidGameActor> toBeAdded = new ArrayList<AndroidGameActor>();
		Actor sons;
		...
		while (running && loaded ) {
			...
			try {
				...
				for ( Actor baseActor: world.getActorList() ) {
					...
					if (sons != null) {
						toBeAdded.add( (AndroidGameActor)sons);
					}
				}
				
				int newId = 0;
				
				for ( AndroidGameActor spawn : toBeAdded ) {
	...
	private void findBestFitForActorFromSector(AndroidGameActor actor,
			GameSector originalSector) {
	...
	public void setDelegate(GameDelegate delegate) {
		...
		for ( Actor actor : world.getActorList() ) {
			( ( AndroidGameActor ) actor ).setGameDelegate( delegate );
		}
	...
	public void restoreState() throws FileNotFoundException, IOException {
		world.loadSnapshotAt( PlayGameActivity.getInstance().openFileInput( "state" ) );		
	}
