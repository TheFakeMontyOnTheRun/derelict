...
import br.odb.derelict.engine.bzk3.android.geometry.GLES1Square;
import br.odb.derelict.engine.bzk3.android.geometry.GLES1Triangle;
import br.odb.derelict.engine.bzk3.android.geometry.GLES1TriangleFactory;
import br.odb.derelict.engine.bzk3.android.geometry.GLESMesh;
...
//		openAllDoors();
	...
	// ------------------------------------------------------------------------------------------------------------
	public void onSectorLeftBy(AndroidGameActor actor) {
...
//		}
	...
	private GLESMesh applyToFace(int face, Decal decal, float scale ) {

		Decal toReturn = new Decal();

		for (int c = 0; c < decal.faces.size(); ++c) {
			toReturn.addFace(this.applyToFace(face,
					(GLES1Triangle) decal.faces.get(c), scale ) );
		}
	...
	public GLES1Triangle applyToFace(int faceNum, GLES1Triangle face, float scale ) {
		return applyToFace(faceNum, face, 0.0f, scale );
	}

	private GLES1Triangle applyToFace(int faceNum, GLES1Triangle face,
		...
		toReturn = GLES1TriangleFactory.getInstance().makeTrig(x0, y0, z0, x1,
